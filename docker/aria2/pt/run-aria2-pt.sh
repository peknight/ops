#!/bin/bash
target_dir=$HOME/software/aria2/data/pt
if [ ! -d "$target_dir" ]; then
  mkdir -p $target_dir
fi
conf_dir=$HOME/software/aria2/etc/pt
if [ ! -d "$conf_dir" ]; then
  mkdir -p $conf_dir
fi
...
docker run -d --name pek-aria2-pt -h pek-aria2-pt -v $conf_dir:/home/$(id -nu)/.aria2 -v $target_dir:/data --net host --cap-add NET_ADMIN pek/webui-aria2-pt
