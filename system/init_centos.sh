#!/bin/bash

# 修改主机名 ubuntu修改/etc/hostname文件
host_name=pek-txbj03-01
echo "$host_name" > /etc/hostname
hostname $host_name

# 修改host文件 追加localhost一行
#vim /etc/hosts
#127.0.0.1       localhost $host_name

# 安装更新
yum -y update

# 安装Docker
curl -sSL https://get.docker.com/ > docker.sh
sh docker.sh

# 安装必要软件
# yum -y curl netstat vim wget
yum -y groupinstall "Development Tools"
yum -y install emacs zsh
# 更新
yum -y update
# 重启docker
systemctl restart docker

# 全局PATH变量加.
#echo "export PATH=.:$PATH" >> /etc/profile

# 设置默认编辑器，选4
#update-alternatives --config editor

# 添加用户组
groupadd -g 612 pek
groupadd -g 1000 server
groupadd shared
groupadd tauriel

# 添加pek用户
useradd -c "peknight" -g pek -G adm,cdrom,shared,docker,server -m -s /bin/bash -u 612 pek
echo 'pek   ALL=(ALL)  NOPASSWD:ALL' | sudo EDITOR='tee -a' visudo
# 设置密码
#passwd pek

# 添加server用户
useradd -c "server" -g server -G adm,cdrom,shared,docker -m -s /bin/bash -u 1000 server
echo 'server ALL=(ALL) ALL' | sudo EDITOR='tee -a' visudo
# 设置密码
#passwd server

# 添加用户
useradd -c "tauriel" -g tauriel -G adm,cdrom,shared,docker,server -m -s /bin/bash tauriel
echo 'tauriel ALL=(ALL) ALL' | sudo EDITOR='tee -a' visudo
#passwd tauriel

# 切换用户
#su - pek

# 创建.ssh目录
home_dir=/home/pek
mkdir -p $home_dir/.ssh

echo "ssh-rsa  peknight@peknight-pc" > $home_dir/.ssh/authorized_keys
echo "ssh-rsa  peknight@iphone7" >> $home_dir/.ssh/authorized_keys
echo "ssh-rsa  peknight@ipadpro2" >> $home_dir/.ssh/authorized_keys
echo "ssh-rsa  pek@pek-control" >> $home_dir/.ssh/authorized_keys
echo "ssh-rsa  pek@pek-sgp1-00" >> $home_dir/.ssh/authorized_keys
echo "ssh-rsa  pek@pek-nyc3-00" >> $home_dir/.ssh/authorized_keys
echo "ssh-rsa  rengang@joinquant.com" >> $home_dir/.ssh/authorized_keys

# 设置密钥对$HOME/.ssh/id_rsa $HOME/.ssh/id_rsa.pub
#ssh-keygen -C "pek@pek-txbj03-01"

# 设置.ssh目录权限
chmod 700 $home_dir/.ssh
chmod 400 $home_dir/.ssh/*

# 设置git
git config --global user.name "peknight"
git config --global user.email "JKpeknight@gmail.com"
git clone git@github.com:peknight/pek_conf.git

# 软连接.vimrc
ln -s $home_dir/pek_conf/vim/vimrc $home_dir/.vimrc

# 设置环境变量
echo "export PATH=\$HOME/bin/ssh:\$PATH" >> $home_dir/.bashrc
source $home_dir/.bashrc

#su - tauriel
home_dir=/home/tauriel
mkdir -p $home_dir/.ssh
echo "ssh-rsa  tauriel@pek-control" > $home_dir/.ssh/authorized_keys
#ssh-keygen -C "tauriel@pek-txbj03-01"
# 设置.ssh目录权限
chmod 700 $home_dir/.ssh
chmod 400 $home_dir/.ssh/*

#su - server
home_dir=/home/server
mkdir -p $home_dir/.ssh
#ssh-keygen -C "server@pek-txbj03-01"
# 设置.ssh目录权限
chmod 700 $home_dir/.ssh
chmod 400 $home_dir/.ssh/*

# 修改sshd_config文件 修改ssh端口号 注意开通8612端口号的防火墙
#vim /etc/ssh/sshd_config
#Port 8612
#PermitRootLogin no
#PasswordAuthentication no
#service sshd restart

